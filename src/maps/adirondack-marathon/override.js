/* adirondack-marathon · override.js
 *
 * Two-distance road race around Schroon Lake, NY (Sun Sep 27, 2026): the
 * Marathon is one full loop (26.8 mi) and the Half Marathon is its exact
 * back half (14.3 mi), starting at the Hamlet of Adirondack and finishing
 * with the marathon at the Schroon Public Beach. The half is a sub-path of
 * the marathon, so aid stations carry a per-loop mile (see aidMile()).
 *
...
// Schroon Lake sits well inside the FSS R2 PMTiles' US northeast
// coverage box (the southeastern Adirondacks — full lake-road network +
// waterways at z14+). Use the same protomaps style the rest of the
// catalog uses, with the cool wooded basemap flavor overrides applied in
// map.on('load') to lean into the navy + lake-blue brand.
//
// AWS Terrarium hillshade is stacked on top in map.on('load') for the
// lake-basin terrain — the east-shore road (miles 4-12) climbs and rolls
// through the hills above Schroon Lake.
...
var map;
var aidOn = true;
// 3D terrain default is OFF. Runners need road labels for orientation,
// and pitch + terrain dims the MapLibre symbol layer on the lake's hills
// (per [[race-map-3d-terrain-marker-occlusion]]). Hillshade alone
// carries the lake-basin topography. 3D is one click away in the Layers
...
// Aid stations are positioned by mile along the *currently selected* loop.
// Each station may carry a per-loop mile map (mileByLoop) — needed when a
// distance is a sub-path of another (the half marathon is the marathon's
// back half, so its miles are offset by the difference in course lengths).
// Falls back to the single `mile` (or kilometer) when no per-loop value is
// present, so this is a no-op for races that don't set mileByLoop.
function aidMile(stn) {
  if (stn.mileByLoop && stn.mileByLoop[currentRaceId] != null) return stn.mileByLoop[currentRaceId];
  if (stn.mile != null) return stn.mile;
  if (stn.kilometer != null) return stn.kilometer / KM_PER_MI;
  return null;
}
...
function addLoopLayers() {
  ...
  LOOP_IDS.forEach(function(id) {
    ...
    map.addLayer({
      ...
      paint: {
        'line-color': '#1A1A1A',
        // Trimmed ~25% from the gran-fondo cycling widths — the lake-road
        // network is denser than the Drumheller highway loops, so a leaner
        // line keeps the route reading without smothering the road names
        // beneath.
...
function updateHqStartLayer(race) {
  ...
  // so there's no gap between the start pennant and the line that leaves
  // it. For the marathon, trkpt[0] is on Main Street (Route 9) in the
  // village; for the half, it's the Hamlet of Adirondack. The pennant
  // pins to whichever loop is active, so the popup names it accordingly.
  ...
  var isHalf = (currentRaceId === 'half-marathon');
  var startName = isHalf ? 'Hamlet of Adirondack · Half Start' : 'Schroon Lake · Start / Finish';
  var startDesc = isHalf
    ? 'Half marathon starts here at 9:00 AM — the marathon’s mile-13.1 marker.'
    : 'Marathon starts on Main Street (Route 9) at 8:00 AM; finish at the Schroon Public Beach.';
  hqStartPopupHtml =
    '<div class="aid-popup">' +
      '<div class="aid-popup__name">' + startName + '</div>' +
      '<div class="aid-popup__km">Mile 0 · ' + race.miles.toFixed(1) + ' mi</div>' +
      '<div class="aid-popup__stocked">' + startDesc + '</div>' +
    '</div>';
  src.setData({
  ...
  idxs.forEach(function(i, n) {
    ...
    var popupHtml =
      '<div class="aid-popup">' +
        ...
        '<div class="aid-popup__km">Mile ' + (raceMile != null ? raceMile.toFixed(1) : '—') + '</div>' +
...
function renderDirectionsList() {
  ...
  ol.onclick = function(ev) {
    ...
    if (kind === 'turn') {
      ...
      // Hazard / water / landmark — point cues. Mark the row active so the
      // selection always reads, clear the (turn-only) segment highlight,
      // and move the camera ONLY when "zoom to step" is checked. The
      // zoom-to-step toggle gates the camera for EVERY directional node —
      // turns and point cues alike — not just the calculated turns.
      clearActiveSegment(); // clear any prior turn segment + active row first
      li.classList.add('loop-cue--active');
      activeRowEl = li;
      if (zoomToStep) {
...
var zoomToStep = (function() {
  try {
    var saved = localStorage.getItem('adirondackMarathon.zoomToStep');
...
function setZoomToStep(on) {
  zoomToStep = !!on;
  try { localStorage.setItem('adirondackMarathon.zoomToStep', zoomToStep ? '1' : '0'); } catch (e) { /* noop */ }
...
function updateAidTable() {
  ...
  var html = AID_STATIONS_ALL.map(function(stn, i) {
    ...
    return '<tr class="' + (skipped ? 'aid-row--skipped' : 'aid-row--visited') + '">' +
      '<td class="aid-col-km">' + (function() { var m = aidMile(stn); return m != null ? m.toFixed(1) : '—'; })() + '</td>' +
...
function drawSimCourse(atMi) {
  ...
  // Start marker (the village for the marathon, the Hamlet of Adirondack
  // for the half — drawn at the active loop's first coordinate)
...
function initSimulator() {
  ...
  simInitialized = true;
  initSimRaces();
  // Default goal time per active race — 11:00 min/mi (a friendly target
  // well inside the marathon's ~7.5-hour course-close window).
  var race = RACES[currentRaceId];
  if (race) {
