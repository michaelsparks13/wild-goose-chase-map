...
// Pocantico Hills Marathon & Half Marathon. The host-supplied GPX has
// no <ele> tags (race director exported lat/lon only), so this script
// fetches per-coordinate elevation from the Open-Meteo Elevation API
// (free, no API key, batches of 100) and emits the same schema the
...
//
// Reads `data/<id>.gpx` directly so the script is self-contained and
// can be rerun any time the host updates the official course GPX.
//
// Run once from the repo root:
//   node scripts/build-pocantico-hills-courses.js

const fs = require('fs');
const path = require('path');
const https = require('https');

const SRC = path.join(__dirname, '..', 'src', 'maps', 'pocantico-hills', 'data');

const DISTANCES = [
  { id: 'marathon',      label: 'Pocantico Hills Marathon',      color: '#34421e' },
  { id: 'half-marathon', label: 'Pocantico Hills Half Marathon', color: '#c19434' },
];
...
async function fetchElevationsMeters(coords) {
  ...
  for (let i = 0; i < coords.length; i += batchSize) {
    ...
    // Heavier courtesy delay (1.2s) so a 900-pt course stays well under
    // the minute budget. Bursting was tripping the limiter.
...
async function buildOne(d) {
  ...
  // 2D dedup + DP simplify FIRST (saves elevation API quota — original
  // file has 1000s of dup points from GPX export, only ~600 unique).
